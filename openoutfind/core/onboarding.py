# openoutfind/core/onboarding.py
...
from openoutfind.core import onboarding_wizard as wiz
...
_INTRO = """
  ...
  OpenOutFind does not send email. The result prints as CSV with the column
  ...
  OpenOutFind is free; you pay only the providers above. Stop anytime — setup
  resumes where you left off.
"""

# The canonical Legal Notice — the single source of truth for how OpenOutFind
...
def _run_bettercontact() -> None:
    from openoutfind.core.models import SiteConfig
    from openoutfind.enrichment.bettercontact import SIGNUP_URL
    ...
    print(
        ...
        "  Get a key (affiliate link — supports OpenOutFind, no markup to you):\n"
        f"  {SIGNUP_URL}\n"
...
def _run_account() -> None:
    ...
    newsletter = _required(wiz.confirm(
        "Subscribe to the OpenOutFind newsletter?",
...
def _require_legal() -> None:
    ...
    while True:
        ...
        if accepted:
            return
        _say("  You must accept the Legal Notice to use OpenOutFind.", "fg:red")
